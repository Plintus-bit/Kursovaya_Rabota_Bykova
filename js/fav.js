        ...
        if (new_fav.IDENT === 'in_fav') {
            ...
            if (new_fav.fav === 1) {
                ...
                for (let i = 3; i <= parseInt(localStorage.getItem('favourite')); i = i + 2) {
                    ...
                    if (new_fav.name === temp_obj.name) {
                        del(i);
                        for (let j = 0; j < element_in_fav_now; ++j) {
                            if (new_fav.name === array[j].querySelector('.h4').textContent) {
...
function del(i) {
    if (i === parseInt(localStorage.getItem('favourite')) - 1) {
        localStorage.removeItem(`${i}`);
        localStorage.removeItem(`${i - 2}`);
        let all_fav = parseInt(localStorage.getItem('favourite')) - 4;
